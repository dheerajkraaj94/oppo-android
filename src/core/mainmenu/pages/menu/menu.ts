...
export class CoreMainMenuPage implements OnDestroy {
    ...
    constructor(private menuDelegate: CoreMainMenuDelegate, private sitesProvider: CoreSitesProvider, navParams: NavParams,
            ...
            ,protected vcProvider: AddonModVedificVCProvider, private alertCtrl: AlertController, private urlHelper: AddonModUrlHelperProvider) {
    ...
    ionViewDidLoad(): void {
        ...
        // hit our custom api here
        this.vcProvider.getAppVersion().then((data) => {
    
        let exitApp = this.vcProvider.forceToExitApp(data);
        if(exitApp){
            
            let app_url:string = this.vcProvider.getAppURL(data);
            let alert = this.alertCtrl.create({
                title: 'Application Updated',
                message: 'A new version has been released. Please update the app.',
